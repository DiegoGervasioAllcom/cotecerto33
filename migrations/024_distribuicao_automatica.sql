-- ============================================================
-- 024: Distribuição automática — trigger BEFORE INSERT em leads
-- ============================================================
...
begin
  ...
  if v_empresa is null and v_modo = 'fila' then
    select e.id into v_empresa
      ...
      left join public.leads l on l.empresa_id = e.id
        and l.status_pipeline in ('novo','qualificando','cotando')
        and coalesce(l.arquivado,false) = false
     where e.status = 'aprovada' and e.tipo <> 'matriz'
     group by e.id
  ...
  if v_empresa is null then
    select id into v_empresa
     ...
     where status = 'aprovada' and tipo <> 'matriz'
     order by random()
     limit 1;
  end if;

  if v_empresa is null then
    return new; -- nenhuma franquia cadastrada; permanece na matriz
  end if;

  -- Escolhe um vendedor aprovado da empresa (rodízio por menor carga)
...
-- Função utilitária: roda a distribuição na fila pendente da matriz
...
declare
  ...
  v_count integer := 0;
  v_cfg record;
  v_uf text; v_cidade text; v_empresa uuid; v_resp uuid; v_modo text;
begin
  ...
  loop
    ...
    if v_empresa is null and v_modo='fila' then
      select e.id into v_empresa from public.empresas e
        left join public.leads l on l.empresa_id=e.id and l.status_pipeline in ('novo','qualificando','cotando') and coalesce(l.arquivado,false)=false
       where e.status='aprovada' and e.tipo<>'matriz'
    ...
    if v_empresa is null then
      select id into v_empresa from public.empresas where status='aprovada' and tipo<>'matriz' order by random() limit 1;
    end if;
    if v_empresa is null then continue; end if;

    select p.id into v_resp from public.profiles p
      left join public.leads l on l.responsavel_id=p.id and l.status_pipeline in ('novo','qualificando','cotando') and coalesce(l.arquivado,false)=false
     where p.empresa_id=v_empresa and p.status='aprovada'
