...
> Documento vivo, gerado em 04/08/2026. Cobre arquitetura, frontend, domínios de
> negócio, testes, deploy e o banco de dados completo (capítulo final,
> `docs/DOC_BANCO_V11.md`). Para o histórico de decisões por frente, ver os
...
franquia, com hierarquia de 7 perfis + 7 cargos, 58 tabelas + 5 views no
...
10. [Banco de dados](#10-banco-de-dados) (capítulo completo, gerado por introspecção)
...
| Infra | Docker (imagem do app publicada no GHCR a cada push em `main`), nginx + certbot, AWS EC2 único, Cloudflare na frente |
...
tela. E-mail transacional (boas-vindas, pendência, recusa, criar senha) está
em produção desde 01/08/2026 via Resend + GoTrue self-hosted.
...
| --- | --- | --- | --- |
| Banco | Vitest + client autenticado por persona | RLS positivo/negativo, RPCs, constraints, triggers | `tests/db/` (62 arquivos) |
| Unitário | Vitest | Lógica pura de front (cálculos, formatação, hooks isolados) | `tests/unit/` (21 arquivos) |
| E2E | Playwright | Fluxo completo no navegador, por perfil | `tests/e2e/` (12 specs) |
...
1. CI publica a imagem no GHCR a cada push em `main` (`ghcr.io/.../cotecerto33:sha-<commit>`) — nunca `:latest` como referência de rollback.
...
4. Smoke test manual pós-deploy: aprovar um acesso, confirmar e-mail único, criar senha, login.
...
| `docs/DOC_BANCO_V11.md` | Banco de dados completo (embutido no capítulo 10) |
...
> Capítulo gerado por introspecção direta do Postgres local (`supabase db reset` limpo, 131 migrations + seed, 04/08/2026) — não é uma cópia manual das migrations, é o schema real. Fonte: `docs/DOC_BANCO_V11.md` (mesmo conteúdo, arquivo próprio para quem quiser só o banco).
