...
> **Revisão documental de 12/08/2026:** os marcadores abaixo descrevem entregas
> versionadas/mergeadas e validações registradas no repositório. Menções históricas a
> produção foram preservadas com a data de sua evidência original, mas não equivalem a
> uma nova verificação do ambiente nesta revisão.
...
| V11.2.10 | banco+front | ✅ Reenvio seguro do primeiro acesso ainda não ativado: nova emissão invalida a anterior, com claim/fencing contra concorrência e sem persistir/exibir o link                              | V11.2.2          |
| V11.2.11 | front       | ✅ Recuperação de senha pelo login, resposta que não enumera contas, redefinição por link e tratamento de link inválido/expirado                                                           | V11.2.2          |
...
**Estado atualizado em 12/08/2026:** Frente 5c mergeada na `main` pelos PRs #138 e
#139, com correções complementares nos PRs seguintes (incluindo #141). As validações
locais registradas acima permanecem como evidência técnica; o deploy das migrations e
da aplicação em produção não foi comprovado nesta revisão documental.
...
| V11.9.5 | banco+testes       | ✅ Integração de entrada do Captação Movida: RPC idempotente recebe o webhook, valida o evento e cria o lead na fila global sem vendedor/empresa forçados                                                                                                                                                                                                                                                                                                                       | —                           |
| V11.9.6 | banco+front+testes | ✅ **Implementada e validada localmente em 14/08/2026:** roteamento dos leads da Captação Movida por loja externa para um pool configurável de vendedores da empresa de destino. Suporta vendedor único, peso, limite diário, exigência de presença, aliases explícitos, auditoria e fallback para a Fila Global. Aplicação/deploy em produção ainda não comprovados.                                                                                                           | V11.9.5                     |
| V11.9.7 | front+testes       | ✅ **Implementada e validada localmente em 16/08/2026:** o passo 6 usa as condições de `quiver_resultado_raw.cards[].premiosPorFormaPagamento`; trocar forma ou condição atualiza o card e transmite exatamente a opção real, inclusive parcelas vazias, sem inferir valores ausentes. Deploy em produção ainda não comprovado.                                                                                                                                                 | integração Quiver existente |
| V11.9.8 | front+testes       | ✅ **Implementada e validada localmente em 16/08/2026:** renovação silenciosa em `TOKEN_REFRESHED`/`USER_UPDATED`, sem limpar o contexto ou exibir loading global; preserva revalidação de acesso, proteção contra corridas/logout/unmount e expulsão de usuários desligados. Deploy em produção ainda não comprovado.                                                                                                                                                          | autenticação existente      |

> **V11.9.7 — evidência e aceite (16/08/2026).** Uma consulta relatada do payload
> armazenado em produção confirmou `premiosPorFormaPagamento[].formaPagamento` e
> opções com `avista`/`parcelas` (por exemplo, `10x sem juros de R$ 306,63`). O campo separado
> `valorParcela` não veio preenchido; portanto a implementação não deve recalcular nem
> inventar esse valor. A task estará concluída quando: (1) não existir lista fixa de
> parcelas; (2) os dois seletores mostrarem somente grupos/opções reais; (3) a troca
> atualizar imediatamente apenas o card correspondente, inclusive total, parcelas,
> franquia e desconto; (4) a transmissão enviar os textos exatos escolhidos; e (5)
> payload legado/incompleto permanecer visível, mas não permitir combinação inexistente.
> Não requer migration nem alteração no robô. Implementação provável em
> `StepCalculo.tsx` e `quiver-resultado.ts`, com testes unitários, de interação e da
> transmissão. Implementação e testes concluídos localmente em 16/08/2026; deploy em
> produção ainda não comprovado. Estimativa original: **4–6h**.

> **V11.9.8 — aceite.** Voltar à aba ou renovar o token mantém URL, conteúdo e árvore
> autenticada montados. `INITIAL_SESSION` e troca real de usuário continuam com carga
> bloqueante; `SIGNED_OUT` limpa imediatamente; refresh e atualização do mesmo usuário
> revalidam em segundo plano. Respostas antigas nunca restauram contexto após logout ou
> troca de identidade, e usuário suspenso/desligado continua perdendo acesso e recebendo
> a mensagem vigente. Não requer migration nem mudança no GoTrue. Estimativa: **4–6h**.
