...
/**
 * Lógica de negócio de `consultarPlaca`, separada da fiação do
 * `createServerFn` para poder ser testada direto (ver
 * `tests/db/placa-server-fn.test.ts`): chamar a função exportada pelo
 * `createServerFn` fora do plugin de bundler do TanStack Start executa o
 * handler de verdade, mas o wrapper de middleware do framework perde o
 * valor de retorno nesse caminho (só reconhece uma propriedade `result`
 * explícita nesse ponto) — chamar esta função pura direto evita a
 * armadilha sem depender desse detalhe do framework.
 */
export async function executarConsultaPlaca(
  data: ConsultarPlacaPayload,
): Promise<ConsultaPlacaResposta> {
  if (!data?.caller_token) throw new Error("Sem token.");
  // Normaliza/valida de novo aqui (não só no `.inputValidator` do
  // createServerFn abaixo): quem chama esta função direto — como os
  // testes de banco — não passa pelo validator, que só roda no branch
  // "server" do wrapper.
...
export const consultarPlaca = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<ConsultaPlacaResposta> => executarConsultaPlaca(data));
