    ...
    <AppShell title="Avaliação de perdas">
      <div className="page-head">
        <div>
          ...
          <div className="sub">Leads marcados como perda pelos vendedores aguardando decisão da matriz.</div>
        </div>
      </div>

      {err && <div className="card" style={{ borderColor: "var(--danger)" }}><div className="card-b">{err}</div></div>}
      {loading ? (
        <div className="card"><div className="card-b">Carregando…</div></div>
      ) : rows.length === 0 ? (
        <div className="card"><div className="card-b">Nenhuma perda pendente de avaliação. 🎉</div></div>
      ) : (
          ...
          {rows.map((r) => (
            <div key={r.id} className="card">
              <div className="card-h">
                <h3>
                  {r.nome} <span className="chip chip-outline" style={{ marginLeft: 8 }}>{money(r.valor)}</span>
                </h3>
                <span className="chip chip-alert">Sugerido: {r.destino_perda_sugerido ?? "—"}</span>
              </div>
              <div className="card-b">
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
                  <div><div className="small muted">Vendedor</div><div>{r.responsavel?.nome ?? "—"}</div></div>
                  <div><div className="small muted">Contato</div><div>{r.contato ?? "—"}</div></div>
                  <div><div className="small muted">Perdido em</div><div>{r.perdida_em ? new Date(r.perdida_em).toLocaleString("pt-BR") : "—"}</div></div>
                  <div><div className="small muted">Motivo</div><div>{r.motivo_perda ?? "—"}</div></div>
                  <div style={{ gridColumn: "span 2" }}><div className="small muted">Sub-motivo</div><div>{r.submotivo_perda ?? "—"}</div></div>
                </div>
                {r.observacao_perda && (
                ...
                <div className="small muted" style={{ marginBottom: 4 }}>Observação da matriz (opcional)</div>
                <textarea
                  className="input"
                  rows={2}
                  value={obs[r.id] ?? ""}
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button className="btn btn-slate" disabled={!!busy} onClick={() => decidir(r.id, "Remalho")}>
                    {busy === r.id + "Remalho" ? "Enviando…" : "Enviar para Remalho"}
                  </button>
                  <button className="btn btn-ghost" disabled={!!busy} onClick={() => decidir(r.id, "Descarte")}>
                    {busy === r.id + "Descarte" ? "Enviando…" : "Confirmar Descarte"}
                  </button>
                  <button className="btn btn-yellow" disabled={!!busy} onClick={() => decidir(r.id, "Reativar")}>
