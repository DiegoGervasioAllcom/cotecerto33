...
function Page() {
  ...
  const filtered = useMemo(() => {
    return rows.filter((p) => {
      const t = classify(p);
      const tabOk =
        tab === "emitidas" ? t === "pagas" || t === "naopagas" : t === tab;
  ...
  function exportCsv() {
    const headers = [
      "Apólice/Proposta", "Seguradora", "Segurado", "CPF/CNPJ", "Tipo",
      "Prêmio", "Comissão", "Vendedor", "Franquia", "Pagamento",
      "Emissão", "Baixa", "Status",
    ];
    ...
    for (const p of filtered) {
      ...
      const statusLbl = status === "pagas" ? "Paga" : status === "naopagas" ? "Não paga"
        : status === "canceladas" ? "Cancelada" : status === "transmissao" ? "Em transmissão" : "Emitida";
      lines.push([
    ...
    <AppShell title="Controle de Vendas">
      ...
      <div className="page-head">
        <div>
          ...
          <div className="sub">
            O <strong>extrato geral da operação</strong> — da transmissão à baixa financeira, num só lugar
          </div>
        </div>
        <div className="tools">
          <select
          ...
          <button className="btn btn-ghost" onClick={exportCsv}>
            <svg width={14} height={14}><use href="#i-download" /></svg> Exportar
          </button>
        </div>
      </div>

      <div className="summary-chips">
        <div className="sum-chip"><span className="sc-val">{counts.transmissao.n}</span><span className="sc-lbl">Em transmissão</span></div>
        <div className="sum-chip info"><span className="sc-val">{counts.emitidas.n}</span><span className="sc-lbl">Emitidas · {fmtBRL(counts.emitidas.total)}</span></div>
        <div className="sum-chip ok"><span className="sc-val">{counts.pagas.n}</span><span className="sc-lbl">Pagas · {fmtBRL(counts.pagas.total)}</span></div>
        <div className="sum-chip alert"><span className="sc-val">{counts.naopagas.n}</span><span className="sc-lbl">Não pagas · {fmtBRL(counts.naopagas.total)}</span></div>
        <div className="sum-chip"><span className="sc-val">{counts.canceladas.n}</span><span className="sc-lbl">Canceladas · {fmtBRL(counts.canceladas.total)}</span></div>
      </div>
      ...
      {!loading && (
        <div style={{ overflowX: "auto" }}>
          <table className="table-pipe mtable" style={{ minWidth: 1180 }}>
            <thead>
              <tr>
                <th>Apólice</th><th>Seguradora</th><th>Segurado</th><th>CPF/CNPJ</th>
                <th>Tipo</th><th>Prêmio</th><th>Comissão</th><th>Vendedor</th>
                <th>Franquia</th><th>Pagamento</th><th>Emissão</th><th>Baixa</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((p) => {
                const seg = p.cotacoes?.segurado?.[0];
                const tipo = (p.tipo_venda || "novo").toLowerCase() === "renovacao" || (p.tipo_venda || "").toLowerCase() === "renovação" ? "Renovação" : "Novo";
                const t = classify(p);
                const chip = t === "pagas" ? ["chip-ok", "Paga"]
                  : t === "naopagas" ? ["chip-alert", "Não paga"]
                  : t === "canceladas" ? ["chip", "Cancelada"]
                  : ["chip-info", "Em transmissão"];
                return (
                  <tr key={p.id}>
                    <td><strong>{p.apolice_numero || p.numero || "—"}</strong></td>
                    <td>{p.seguradora || "—"}</td>
                    <td>
                    ...
                    <td><small className="muted">{seg?.cpf_cnpj ? maskCpfCnpj(seg.cpf_cnpj) : "—"}</small></td>
                    <td><span className={`chip ${tipo === "Renovação" ? "chip-info" : "chip-yellow"}`}>{tipo}</span></td>
                    <td>{fmtBRL(Number(p.premio ?? p.valor ?? 0))}</td>
                    <td><strong>{fmtBRL(Number(p.comissao_valor ?? 0))}</strong></td>
                    <td><small>{profiles[p.responsavel_id || ""]?.nome || "—"}</small></td>
                    <td><small>{empresas[p.empresa_id || ""]?.nome || "—"}</small></td>
                    <td><small>{p.forma_pagamento || "—"}</small></td>
                    <td><small className="muted">{fmtDate(p.emitida_em)}</small></td>
                    <td><small className="muted">{fmtDate(p.baixa_em)}</small></td>
                    <td><span className={`chip ${chip[0]}`}>{chip[1]}</span></td>
                  </tr>
                );
              })}
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={13} style={{ padding: 28, textAlign: "center", color: "var(--muted)" }}>
