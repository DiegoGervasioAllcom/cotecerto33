...
function Page() {
  ...
  const landing = resolverLanding({ role, isGroupView, groupLoading });

  useEffect(() => {
    if (!session?.user?.id || landing !== "/inicio") return;
    void carregar(session.user.id).then((r) => {
      setD(r);
      setLoading(false);
    });
  }, [session?.user?.id, landing]);

  if (!landing) return <div className="muted">Carregando…</div>;
  if (landing === "/comando/visao-geral") return <Navigate to={landing} replace />;
  ...
  const pct =
    d && d.metaApolices ? Math.min(100, Math.round((d.apolices / d.metaApolices) * 100)) : 0;
  const faltam = d ? Math.max(0, d.metaApolices - d.apolices) : 0;
  const ritmoBadge =
    pct >= 67
      ? { cls: "ritmo-ok", txt: "No ritmo" }
      : pct >= 33
        ? { cls: "ritmo-ok", txt: "Avançando" }
        : { cls: "ritmo-ok", txt: "Comece já" };
  const diasUteis = diasUteisRestantes(today);
  const ritmo =
    faltam && diasUteis ? `${Math.max(1, Math.round(diasUteis / Math.max(1, faltam)))}` : "—";
  const previsao =
    d && d.apolices ? Math.round((d.premioMes / d.apolices) * d.metaApolices) : d?.premioMes || 0;
  ...
  const trend = dash.tendencia.length
    ...
    : Array.from({ length: 7 }, (_, i) => ({
        dia: ["Sex", "Sáb", "Dom", "Seg", "Ter", "Qua", "Qui"][i],
        leads: 0,
        cotacoes: 0,
        fechados: 0,
      }));
    ...
    <AppShell title="Início">
      ...
      <div className="page-head">
        <div>
          ...
          <div className="sub">
            {headerSub} {loading ? "Carregando…" : "Bora fechar a meta! 💪"}
          </div>
        </div>
        <div className="tools">
          ...
          <div className="hero-placar">
            <div className="ring-wrap">
              ...
              <div className="ring-txt">
                ...
                <div className="ring-sub">da meta</div>
              </div>
            </div>
            <div className="hero-stats">
              <div className="hero-stat">
                ...
                <div className="hs-sub">
                  {dash.metaApolices ? (
                    <>
                      faltam <strong>{faltam}</strong> pra meta
                    </>
                  ) : (
                    <>defina a meta em metas mensais</>
                  )}
                </div>
              </div>
              <div className="hero-stat">
                <div className="hs-lbl">PRÊMIO NO MÊS</div>
                <div className="hs-val" style={{ color: "var(--ok)" }}>
                  {BRL(dash.premioMes)}
                </div>
              ...
              <div className="hero-stat">
                ...
                <div className="hs-val">
                  {faltam ? `1 a cada ${ritmo} dia${ritmo === "1" ? "" : "s"}` : "Meta batida"}
                </div>
                <div className="hs-sub">
                  {faltam} apólice{faltam === 1 ? "" : "s"} em {diasUteis} dias
                </div>
              </div>
              <div className="hero-stat">
          ...
          <div className="card" data-tour="home-funil">
            <div className="card-h">
              <h3>
                ...
                Meu funil — onde melhorar
              </h3>
              <span className="small muted">
                {MESES[today.getMonth()].slice(0, 3)}/{String(today.getFullYear()).slice(-2)}
              </span>
            </div>
            <div className="card-b">
              <div className="funnel">
                <div className="funnel-row">
                  <div className="fn-lbl">Leads</div>
                  <div className="funnel-track">
                    <div
                      className="funnel-bar"
                      style={{ width: fnW(dash.funil.leads), background: "#2F3D48" }}
                    >
                      {dash.funil.leads}
                    </div>
                  </div>
                ...
                <div className="funnel-row">
                  <div className="fn-lbl">Cotações</div>
                  <div className="funnel-track">
                    <div
                      className="funnel-bar"
                      style={{ width: fnW(dash.funil.cotacoes), background: "#425563" }}
                    >
                      {dash.funil.cotacoes}
                    </div>
                  </div>
                ...
                <div className="funnel-row">
                  <div className="fn-lbl">Propostas</div>
                  <div className="funnel-track">
                    <div
                      className="funnel-bar"
                      style={{ width: fnW(dash.funil.propostas), background: "#FFB600" }}
                    >
                      {dash.funil.propostas}
                    </div>
                  </div>
                ...
                <div className="funnel-row">
                  <div className="fn-lbl">Fechados</div>
                  <div className="funnel-track">
                    <div
                      className="funnel-bar"
                      style={{ width: fnW(dash.funil.fechados), background: "#2E8B57" }}
                    >
                      {dash.funil.fechados}
                    </div>
                  </div>
        ...
        <div className="col">
          ...
          <div className="card">
            ...
            <div className="card-b">
              <div className="pos-row">
                ...
                <div>
                  <div className="bold" style={{ color: "var(--slate)" }}>
                    de {dash.posicao.total} vendedor{dash.posicao.total === 1 ? "" : "es"}
                  </div>
              ...
              <div style={{ marginTop: 14 }}>
                <div className="row" style={{ justifyContent: "space-between" }}>
                  ...
                  <span className="bold" style={{ color: "var(--slate)" }}>
                    {dash.posicao.minhaConv}%
                  </span>
                </div>
                <div className="posbar">
                  <div
                    style={{
                      width: `${dash.posicao.minhaConv}%`,
                      height: "100%",
                ...
                <div className="row" style={{ justifyContent: "space-between", marginTop: 8 }}>
                  ...
                  <span className="bold" style={{ color: "var(--slate)" }}>
                    {dash.posicao.mediaConv}%
                  </span>
                </div>
                <div className="posbar">
                  <div
                    style={{
                      width: `${dash.posicao.mediaConv}%`,
                      height: "100%",
          ...
          <div className="card">
            <div className="card-h">
              ...
              <span className="small muted">
                {dash.conquistas.filter((c) => c.ok).length}/{dash.conquistas.length}
              </span>
            </div>
            <div className="card-b">
          ...
          <div className="card chart-card" data-tour="home-tendencia">
            <div className="card-h">
              ...
              <div className="chart-legend">
                <div className="it">
                  <div className="dot" style={{ background: "var(--slate)" }} />
                  Leads
                </div>
                <div className="it">
            ...
            <div className="card-b">
              ...
              >
                {[30, 65, 100, 135, 170].map((y) => (
                  <line key={y} x1="30" x2="690" y1={y} y2={y} stroke="#EFEAD9" strokeWidth="1" />
                ))}
...
async function carregar(userId: string): Promise<DashData> {
  ...
  const premioMes = (propsMes ?? []).reduce((s, p) => s + Number(p.premio || 0), 0);
  ...
  if (empresaId) {
    ...
    const sorted = ids.map((id) => ({ id, n: counts.get(id) ?? 0 })).sort((a, b) => b.n - a.n);
    ...
    (leadsEmpresa ?? []).forEach((l) => {
      if (!l.responsavel_id) return;
      leadsPor.set(l.responsavel_id, (leadsPor.get(l.responsavel_id) ?? 0) + 1);
    });
    const minhaConv = leadsPor.get(userId)
      ? Math.round((funil.fechados / (leadsPor.get(userId) || 1)) * 100)
      : 0;
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    dias.push({
      dia: ["Dom", "Seg", "Ter", "Qua", "Qui", "Sex", "Sáb"][dt.getDay()],
      date: dt,
      leads: 0,
      cotacoes: 0,
      fechados: 0,
  ...
  const [{ data: l7 }, { data: c7 }, { data: t7 }] = await Promise.all([
    ...
    supabase
      .from("propostas")
      .select("transmitida_em")
      .eq("responsavel_id", userId)
      .eq("status", "transmitida")
      .gte("transmitida_em", seteDiasAtras),
  ]);
  const bucket = (
    rows: Record<string, string | null>[] | null,
    key: string,
    field: "leads" | "cotacoes" | "fechados",
  ) => {
    (rows ?? []).forEach((r) => {
      const dt = new Date(r[key] as string);
      const ix = dias.findIndex(
        (d) =>
          d.date.toDateString() ===
          new Date(dt.getFullYear(), dt.getMonth(), dt.getDate()).toDateString(),
      );
  ...
  // 7) Conquistas
  const { data: anyVenda } = await supabase
    .from("propostas")
    .select("id")
    .eq("responsavel_id", userId)
    .eq("status", "transmitida")
  ...
  const diasComContato = new Set(
    dias.filter((d) => d.leads + d.cotacoes + d.fechados > 0).map((d) => d.date.toDateString()),
  );
  let seq = 0,
    melhor = 0;
  dias.forEach((d) => {
    if (diasComContato.has(d.date.toDateString())) {
      seq++;
      melhor = Math.max(melhor, seq);
    } else seq = 0;
  });
  const conquistas = [
    ...
    { id: "prop5", icon: "📄", nome: "5 propostas/semana", ok: propsSemana >= 5 },
    {
      id: "meta",
      icon: "🎯",
      nome: "Bater meta do mês",
      ok: metaApolices > 0 && apolices >= metaApolices,
    },
