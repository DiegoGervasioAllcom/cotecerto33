...
const STAGE_META: Record<string, { titulo: string; descricao: string; icon: string }> = {
  ...
  qualificado: {
    titulo: "Qualificado",
    descricao: "perfil e veículo confirmados",
    icon: "i-check",
  },
  cotacao: { titulo: "Cotado", descricao: "multi-cálculo rodado", icon: "i-layers" },
  proposta: {
    titulo: "Proposta enviada",
    descricao: "comparativo enviado ao cliente",
    icon: "i-file",
  },
...
function Page() {
  ...
  const kpis = useMemo(() => {
    const pendentes = enriched.filter(
      (l) => !l.distribuido && l.status_pipeline === "novo" && !l.arquivado,
    );
    ...
    const distDurations = distribuidos.map((l) =>
      Math.max(0, (new Date(l.distribuido_em!).getTime() - new Date(l.criado_em).getTime()) / 1000),
    );
    const avgDist = distDurations.length
      ? Math.round(distDurations.reduce((a, s) => a + s, 0) / distDurations.length)
      : 0;
  ...
  const chips = useMemo(() => {
    ...
    return {
      pendentes: enriched.filter(
        (l) => !l.distribuido && l.status_pipeline === "novo" && !l.arquivado,
      ).length,
      slaOver: enriched.filter(
        (l) =>
          !l.distribuido && l.status_pipeline === "novo" && !l.arquivado && l.ageSec > SLA_SECONDS,
      ).length,
      emAtend: enriched.filter((l) =>
        ["contato", "qualificado", "cotacao", "proposta", "negociacao"].includes(l.status_pipeline),
      ).length,
      filaHoje: enriched.filter((l) => today(l.criado_em)).length,
      fechadosHoje: enriched.filter((l) => l.status_pipeline === "ganho" && today(l.criado_em))
        .length,
    };
  }, [enriched]);
  ...
  async function executarDistribuicaoAuto() {
    if (kpis.pendentes === 0) {
      setDistAutoMsg({ type: "err", text: "Não há leads pendentes na fila." });
      return;
    }
    setDistAutoLoading(true);
    setDistAutoMsg(null);
    const { data: n, error } = await supabase.rpc("distribuir_fila_pendente");
    setDistAutoLoading(false);
    if (error) {
      setDistAutoMsg({ type: "err", text: error.message });
      return;
    }
    const count = typeof n === "number" ? n : 0;
    setDistAutoMsg({
      type: "ok",
      text: `${count} lead${count === 1 ? "" : "s"} distribuído${count === 1 ? "" : "s"} automaticamente.`,
    });
    load();
  }
  ...
  async function toggleArquivar(l: Lead) {
    ...
    if (
      arquivar &&
      !confirm(
        `Arquivar o lead "${l.nome}"? Ele sairá da lista padrão e pode ser visto no filtro "Arquivados".`,
      )
    )
      return;
    const { error } = await supabase.rpc(arquivar ? "arquivar_lead" : "desarquivar_lead", {
      p_lead: l.id,
    });
    if (error) alert(error.message);
    else load();
    ...
    <AppShell title="Leads">
      ...
      <div className="page-head">
        <div>
          ...
          <div className="sub">
            Lead de campanha é mídia paga esfriando — <strong>distribua em até 3 min</strong> ou a
            conversão despenca
          </div>
        </div>
        <div className="tools">
      ...
      {distAutoMsg && (
        <div
          ...
          style={{
            background: distAutoMsg.type === "ok" ? "var(--ok-soft)" : "var(--alert-soft)",
            color: distAutoMsg.type === "ok" ? "var(--ok)" : "var(--alert)",
            marginBottom: 16,
          }}
        >
          <svg width="16" height="16">
            <use href={distAutoMsg.type === "ok" ? "#i-check-circle" : "#i-alert-triangle"}></use>
          </svg>{" "}
          <strong style={{ marginRight: 4 }}>
            {distAutoMsg.type === "ok" ? "Sucesso." : "Erro."}
          </strong>{" "}
          {distAutoMsg.text}
        </div>
      )}
      ...
      {chips.pendentes > 0 && (
        ...
        >
          ...
          <strong style={{ marginRight: 4 }}>Distribuição manual.</strong> {chips.pendentes} leads
          esperando você distribuir.
        </div>
      )}
      ...
      <div className="kpi-grid">
        <div className={`kpi ${kpis.oldest > SLA_SECONDS ? "k-alert" : ""}`}>
          ...
          <div className="val" style={{ fontSize: 24 }}>
            {kpis.oldest > 0 ? fmtDur(kpis.oldest) : "—"}
          </div>
          <div className="meta">
        ...
        <div className="kpi">
          ...
          <div className="val" style={{ fontSize: 24 }}>
            {kpis.avgDist > 0 ? fmtDur(kpis.avgDist) : "—"}
          </div>
        ...
        <div className="kpi">
          ...
          <div className="val" style={{ fontSize: 24 }}>
            {kpis.avgAtend > 0 ? fmtDur(kpis.avgAtend) : "—"}
          </div>
      ...
      <div className="summary-chips">
        <div className="sum-chip alert">
          <span className="sc-val">{chips.pendentes}</span>
          <span className="sc-lbl">Aguardando distribuição</span>
        </div>
        <div className="sum-chip alert">
          <span className="sc-val">{chips.slaOver}</span>
          <span className="sc-lbl">SLA estourado</span>
        </div>
        <div className="sum-chip info">
          <span className="sc-val">{chips.emAtend}</span>
          <span className="sc-lbl">Em atendimento</span>
        </div>
        <div className="sum-chip">
          <span className="sc-val">{chips.filaHoje}</span>
          <span className="sc-lbl">Na fila hoje</span>
        </div>
        <div className="sum-chip ok">
          <span className="sc-val">{chips.fechadosHoje}</span>
      ...
      <div className="filters-bar">
        ...
        <select
          className="select-mini"
          value={fArquivados}
          onChange={(e) => setFArquivados(e.target.value as "ativos" | "arquivados" | "todos")}
        >
        ...
        <span className="small muted">
          fila ordenada por urgência · {filtered.length} de {leads.length} leads
        </span>
      </div>
        ...
        <table className="table-pipe mtable" style={{ minWidth: 1200 }}>
          ...
          <tbody>
            {filtered
              .sort((a, b) => {
                const aPri =
                  !a.distribuido && a.status_pipeline === "novo"
                    ? a.ageSec > SLA_SECONDS
                      ? 0
                      : 1
                    : 2;
                const bPri =
                  !b.distribuido && b.status_pipeline === "novo"
                    ? b.ageSec > SLA_SECONDS
                      ? 0
                      : 1
                    : 2;
              ...
              .map((l) => {
                ...
                const slaPill =
                  !l.distribuido && l.status_pipeline === "novo"
                    ...
                    : {
                        cls: l.slaSec > SLA_SECONDS ? "over" : "ok",
                        txt: `distribuído há ${fmtDur(l.slaSec)}`,
                      };
                  ...
                  <tr key={l.id} style={rowStyle}>
                    ...
                    <td>
                      ...
                      {l.uf && (
                        <>
                          <br />
                          <small className="muted">{l.uf}</small>
                        </>
                      )}
                    </td>
                    <td>
                      <small className="muted">{fmtAgo(l.criado_em)}</small>
                    </td>
                    <td>
                      ...
                      {l.arquivado && (
                        <>
                          ...
                          <span
                            className="chip"
                            style={{ marginTop: 4, background: "#eee", color: "#666" }}
                          >
                            arquivado
                          </span>
                        </>
                    ...
                    <td>
                      {isPerdido ? (
                        <span className="chip" style={{ background: "#eee", color: "#666" }}>
                          encerrado
                        </span>
                      ) : (
                        <span className={`sla-pill ${slaPill.cls}`}>
                      ...
                      >
                        ...
                        {!l.bloqueado && l.distribuido && !isFechado && (
                          <button
                            ...
                            onClick={() => puxarDeVolta(l)}
                          >
                            <svg width="14" height="14">
                              <use href="#i-corner-up-left"></use>
                            </svg>
                          </button>
                        )}
                        <button
                          className="ic-mini"
                          title="Histórico"
                          onClick={() => setModal({ kind: "hist", lead: l })}
                        >
                          <svg width="14" height="14">
                            <use href="#i-clock"></use>
                          </svg>
...
function HistoricoModal({
  ...
  useEffect(() => {
    supabase
      .from("lead_eventos")
      .select("id,tipo,titulo,descricao,criado_em,meta")
      .eq("lead_id", lead.id)
      .order("criado_em")
  ...
  function nomeEmpresa(id?: string | null) {
    return id ? empresas[id]?.nome || "—" : null;
  }
  function nomeVendedor(id?: string | null) {
    return id ? profiles[id]?.nome || "—" : null;
  }
  ...
  const timeline = useMemo(() => {
    ...
    const temEventoDist = eventos.some(
      (e) => e.tipo === "redistribuido" || e.tipo === "lead_assumido",
    );
    if (lead.distribuido_em && !temEventoDist) {
      ...
      items.push({
        titulo: "Distribuído",
        descricao: [
          empresaNome && `Franquia: ${empresaNome}`,
          vendNome ? `Vendedor: ${vendNome}` : "Sem vendedor — fila da franquia",
        ]
          .filter(Boolean)
          .join(" · "),
    ...
    if (lead.ultimo_atendimento_em && !eventos.some((e) => e.tipo === "lead_assumido")) {
      items.push({
        titulo: "1º contato",
        descricao: "primeira conversa com o cliente",
        ts: new Date(lead.ultimo_atendimento_em).getTime(),
        icon: "i-phone",
      });
    }
    const stIdx = STAGE_ORDER.indexOf(lead.status_pipeline);
    if (stIdx >= 0) {
      const passados = ["qualificado", "cotacao", "proposta", "negociacao"].filter(
        (s) => STAGE_ORDER.indexOf(s) <= stIdx,
      );
      ...
      passados.forEach((s, i) => {
        ...
        items.push({
          ...
          ts: baseTs - (passados.length - i) * 3600_000,
          icon: m.icon,
        });
      });
      if (lead.status_pipeline === "ganho" || lead.status_pipeline === "perdido") {
        ...
        items.push({
          titulo: m.titulo,
          descricao:
            lead.status_pipeline === "perdido" ? lead.motivo_perda || m.descricao : m.descricao,
          ts: baseTs,
          icon: m.icon,
        });
      }
    ...
    for (const e of eventos) {
      ...
      const partes: string[] = [];
      if (
        e.tipo === "redistribuido" ||
        e.tipo === "lead_assumido" ||
        e.tipo === "reativado_de_perda"
      ) {
      ...
      items.push({
        ...
        icon: EVENT_ICON[e.tipo] || "i-share",
      });
    }
    items.sort((a, b) => a.ts - b.ts);
    const dur: number[] = items.map((it, i) =>
      i === 0 ? Date.now() - it.ts : it.ts - items[i - 1].ts,
    );
  ...
  return (
    <Modal title={`Linha do tempo — ${lead.nome || "lead"}`} onClose={onClose} icon="i-clock">
      ...
      >
        O tempo gasto em cada etapa mostra onde o lead travou. A etapa em{" "}
        <span style={{ color: "var(--alert)" }}>vermelho</span> é o <strong>maior gargalo</strong>{" "}
        desta jornada.
      </div>
          ...
          <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
            <div
              style={{
                ...
                height: 36,
                borderRadius: "50%",
                background: it.gargalo ? "var(--alert)" : "#f4ead5",
                color: it.gargalo ? "#fff" : "#8a6400",
                display: "flex",
                alignItems: "center",
                justifyContent: "center",
                flexShrink: 0,
            ...
            <span
              className={`chip ${it.gargalo ? "chip-danger" : ""}`}
              style={{ alignSelf: "center" }}
            >
              {i === 0 ? fmtAgo(new Date(it.ts).toISOString()) : fmtDur(it.durSec)}
              {it.gargalo ? " · gargalo" : ""}
            </span>
          </div>
        ))}
...
function RedistModal({
  ...
  return (
    <Modal
      title={`${isRedist ? "Redistribuir" : "Distribuir"} — ${lead.nome || "lead"}`}
      onClose={onClose}
      icon="i-share"
    >
...
function BlockModal({
  ...
  async function salvar() {
    ...
    if (isUnblock) {
      ...
      ({ error } = await supabase.rpc("bloquear_lead", {
        p_lead: lead.id,
  ...
  return (
    <Modal
      title={`${isUnblock ? "Desbloquear" : "Bloquear"} lead — ${lead.nome || ""}`}
      onClose={onClose}
      icon="i-lock"
    >
      {isUnblock ? (
        <p>
          O lead será desbloqueado e voltará a ser elegível para distribuição/atendimento.
          {lead.motivo_bloqueio && (
            <>
              <br />
              <br />
              <strong>Motivo do bloqueio anterior:</strong> {lead.motivo_bloqueio}
            </>
          )}
        </p>
      ) : (
        <>
          <p className="muted" style={{ marginTop: 0 }}>
            Leads bloqueados ficam fora da fila de distribuição. Útil para clientes irregulares,
            duplicidades ou pedidos de exclusão.
          </p>
          ...
          <textarea
            ...
            placeholder="Ex.: cliente solicitou remoção, dados inválidos, duplicidade..."
          />
        </>
      )}
      <div style={{ marginTop: 22, display: "flex", justifyContent: "flex-end", gap: 8 }}>
        ...
        <button
          className={`btn ${isUnblock ? "btn-yellow" : "btn-danger"}`}
          onClick={salvar}
          disabled={saving}
        >
