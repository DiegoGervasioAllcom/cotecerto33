...
function Page() {
  ...
  const [simResult, setSimResult] = useState<
    { leadId: string; nome: string; alvo: string; criterio: string }[] | null
  >(null);
  ...
  async function load() {
    ...
    if (c.data) {
      setCfg({
        id: c.data.id,
        automatico_on: !!c.data.automatico_on,
    ...
    setDevolvidos(
      (d.data ?? []).map((x) => {
        const label = veiculoLabel(x.dados as Record<string, unknown> | null);
        return {
          id: x.id,
          nome: x.nome,
          motivo_perda: x.motivo_perda,
          submotivo_perda: x.submotivo_perda,
          destino_perda_sugerido: x.destino_perda_sugerido,
          observacao_perda: x.observacao_perda,
          responsavel_id: x.responsavel_id ?? null,
          empresa_id: x.empresa_id ?? null,
          atualizado_em: x.atualizado_em ?? null,
          // veiculoLabel devolve "—" quando não há dados; mantém null para esconder a linha.
          veiculo: label === "—" ? null : label,
        };
      }),
    );
    setFila(
      (f.data ?? []).map((x) => {
        const dados = x.dados as { cidade?: string; uf?: string } | null;
        return {
          id: x.id,
          nome: x.nome,
          criado_em: x.criado_em,
          distribuido_em: x.distribuido_em,
    ...
    setFranquias(
      (fr.data ?? [])
        .filter((e) => e.id !== mId && e.status === "aprovada")
        .map((e) => ({
          id: e.id,
          nome: e.nome,
          cidade: e.cidade,
    ...
    const presMap = new Map<string, string>(
      (pr.data ?? [])
        .filter((x): x is { user_id: string; status_efetivo: string } =>
          Boolean(x.user_id && x.status_efetivo),
        )
        .map((x) => [x.user_id, x.status_efetivo]),
    );
    setVendedores(
      (vd.data ?? [])
        .filter((p) => p.empresa_id && p.empresa_id !== mId && p.status === "aprovada")
        .map((p) => ({
          id: p.id,
          nome: p.nome,
  ...
  const tempoMedioManual = useMemo(() => {
    ...
    if (!distribuidosRecentes.length) {
      ...
      const oldest = fila.reduce(
        (a, l) => Math.max(a, Date.now() - new Date(l.criado_em).getTime()),
        0,
      );
      return Math.round(oldest / 1000);
    }
    return Math.round(
      distribuidosRecentes.reduce(
        (a, l) => a + (new Date(l.distribuido_em!).getTime() - new Date(l.criado_em).getTime()),
        0,
      ) /
        distribuidosRecentes.length /
        1000,
    );
  }, [fila]);
  ...
  async function patchCfg(partial: Partial<Config>) {
    ...
    const { error } = await supabase
      ...
      .update({
        automatico_on: next.automatico_on,
        modo: next.modo,
        criterios: next.criterios,
        sla_segundos: next.sla_segundos,
      })
      .eq("id", "default");
  ...
  function simular() {
    ...
    setSimResult(
      fila.slice(0, 20).map((l) => {
        const r = escolherAlvoParaLead(l);
        return {
          leadId: l.id,
          nome: l.nome || `Lead ${l.id.slice(0, 6)}`,
          alvo: r.alvo,
          criterio: r.criterio,
        };
      }),
    ...
    <AppShell title="Distribuição">
      ...
      <div className="page-head">
        <div>
          ...
          <div className="sub">
            Defina a regra que controla como cada lead chega até franquias e vendedores
          </div>
        </div>
        <div className="tools">
      ...
      <div className="card" style={{ marginBottom: 18 }}>
        <div className="card-h">
          <h3>
            ...
            Leads devolvidos pelos vendedores — triagem
          </h3>
          <span className="small muted">
            {devolvidos.length} aguardando · Remalho, Descarte ou Reativar
          </span>
        </div>
        <div className="card-b">
          {devolvidos.length === 0 && (
            <div className="muted small" style={{ padding: 6 }}>
              Nenhum lead devolvido aguardando triagem.
            </div>
          )}
          {devolvidos.map((l) => {
            const vendNome = l.responsavel_id
              ? (vendedores.find((v) => v.id === l.responsavel_id)?.nome ?? "—")
              : "—";
            const franqNome = l.empresa_id
              ? (franquias.find((f) => f.id === l.empresa_id)?.nome ?? "—")
              : "—";
            return (
              <div
                key={l.id}
              ...
              >
                <div
                  className="row"
                  style={{ alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}
                >
                      ...
                      {l.destino_perda_sugerido && (
                        <span
                          ...
                          style={{
                            fontSize: 10,
                            background:
                              l.destino_perda_sugerido === "Descarte"
                                ? "var(--alert-soft)"
                                : "var(--ok-soft)",
                            color:
                              l.destino_perda_sugerido === "Descarte"
                ...
                >
                  ...
                  <button
                    ...
                    disabled={busyId === l.id}
                    onClick={() => setDestinoPerda(l.id, "Reativar")}
                  >
                  ...
                  <button
                    className="btn btn-ghost btn-sm"
                    disabled={busyId === l.id}
                    onClick={() => setDestinoPerda(l.id, "Remalho")}
                  >
                    <svg width="12" height="12">
                      <use href="#i-mail"></use>
                    </svg>{" "}
                  ...
                  <button
                    className="btn btn-ghost btn-sm"
                    disabled={busyId === l.id}
                    onClick={() => setDestinoPerda(l.id, "Descarte")}
                  >
                    <svg width="12" height="12">
          ...
          <div className="audit-note" style={{ marginTop: 12 }}>
            ...
            <strong style={{ margin: "0 4px" }}>Reativar</strong> devolve o lead à fila de
            distribuição.
            <strong style={{ margin: "0 4px" }}>Remalho</strong> e{" "}
            <strong style={{ margin: "0 4px" }}>Descarte</strong> encerram o lead — a decisão fica
      ...
      <div className="card card-yellow" style={{ marginBottom: 18 }}>
        <div className="card-h">
          ...
          <div
            className={`switch ${cfg.automatico_on ? "on" : ""}`}
            onClick={() => patchCfg({ automatico_on: !cfg.automatico_on })}
            style={{ cursor: "pointer" }}
          >
            <span className="track"></span>
            <span className="label">{cfg.automatico_on ? "Ligada" : "Desligada"}</span>
          </div>
        </div>
        <div className="card-b">
          {!cfg.automatico_on ? (
            ...
            >
              ...
              <strong style={{ marginRight: 4 }}>Modo manual ativo.</strong>
              Agora há <strong style={{ margin: "0 4px" }}>{fila.length}</strong> lead
              {fila.length === 1 ? "" : "s"} na fila.
            </div>
          ) : (
            <div
              className="audit-note"
            ...
            >
              ...
              <strong style={{ marginRight: 4 }}>Distribuição automática ligada.</strong>
              Novos leads são distribuídos seguindo a regra abaixo.
            </div>
          )}
          <div
            className="row"
            style={{ gap: 22, marginTop: 16, flexWrap: "wrap", alignItems: "center" }}
          >
            <div>
              <div className="small muted" style={{ fontWeight: 600 }}>
                MANUAL (hoje)
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, color: "var(--alert)" }}>
                {fmtDur(tempoMedioManual)}
              </div>
              <div className="small muted">tempo médio até distribuir</div>
            </div>
            ...
            <div>
              ...
              <div style={{ fontSize: 24, fontWeight: 700, color: "var(--ok)" }}>~8s</div>
              <div className="small muted">distribuição instantânea por regra</div>
            </div>
            <div className="spacer"></div>
            {!cfg.automatico_on && (
              <button
                className="btn btn-yellow"
                onClick={() => patchCfg({ automatico_on: true })}
                disabled={saving}
              >
                <svg width="14" height="14">
      ...
      <div className="card" style={{ marginBottom: 18, opacity: cfg.automatico_on ? 1 : 0.6 }}>
        <div className="card-h">
          ...
          <span className="small muted">ativa quando o automático estiver ligado</span>
        </div>
        <div className="card-b">
          <div className="mode-grid">
            {(
              [
                {
                  k: "regiao",
                  t: "Automático por região",
                  d: "Distribui pela cidade/UF do lead até a franquia mais próxima.",
                  ic: "i-pin",
                },
                {
                  k: "performance",
                  t: "Automático por performance",
                  d: "Prioriza vendedores com melhor conversão e menor tempo de contato.",
                  ic: "i-gauge",
                },
                {
                  k: "fila",
                  t: "Fila equilibrada",
                  d: "Rodízio justo: equilibra o volume de leads entre os vendedores.",
                  ic: "i-refresh",
                },
              ] as const
            ).map((m) => (
              <div
                key={m.k}
                className={`mode-card ${cfg.modo === m.k ? "on" : ""}`}
                onClick={() => patchCfg({ modo: m.k })}
                style={{ cursor: "pointer" }}
              >
                <div className="mc-ic">
      ...
      <div className="detail-grid">
        <div className="card">
          <div className="card-h">
            ...
            <span className="small muted">aplicados na automação</span>
          </div>
          <div className="card-b">
              ...
              {(
                [
                  ...
                  [
                    ...
                    "Respeitar carteira/território de cada franquia",
                  ],
                ...
                ] as [keyof Criterios, string, string][]
              ).map(([k, t, d]) => (
                <div key={k} className="crit-row">
                  <div className="cr-body">
                    <div className="cr-t">{t}</div>
                    <div className="cr-d">{d}</div>
                  </div>
                  <div
                    className={`switch ${cfg.criterios[k] ? "on" : ""}`}
                    onClick={() =>
                      patchCfg({ criterios: { ...cfg.criterios, [k]: !cfg.criterios[k] } })
                    }
                    style={{ cursor: "pointer" }}
                  >
                    <span className="track"></span>
        ...
        <div className="card card-yellow">
          ...
          <div className="card-b">
            <p className="small muted" style={{ marginTop: 0 }}>
              Quando um lead é distribuído, o vendedor tem um tempo para aceitar ou iniciar o
              atendimento. Se não reagir, o lead volta para a fila.
            </p>
            <div className="field-group">
              <label>Tempo para reagir</label>
              <select
                className="input"
                value={String(cfg.sla_segundos)}
                onChange={(e) => patchCfg({ sla_segundos: parseInt(e.target.value, 10) })}
              >
            ...
            <div className="audit-note">
              ...
              O contador aparece para o vendedor no card do lead, na coluna "Novo" do pipeline.
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{ marginTop: 18 }}>
        <div className="card-h">
          ...
          <div className="card-h-actions">
            <button className="btn btn-slate btn-sm" onClick={simular}>
              <svg width="14" height="14">
                <use href="#i-spark"></use>
              </svg>{" "}
              Simular com a fila atual ({fila.length})
            </button>
          </div>
        </div>
        <div className="card-b">
          {simResult === null && (
            <p className="muted small" style={{ margin: 0 }}>
              Rode a simulação para ver, antes de confirmar, para quem cada lead pendente iria com a
              regra acima.
            </p>
          )}
          {simResult !== null && simResult.length === 0 && (
            <p className="muted small" style={{ margin: 0 }}>
              Nenhum lead na fila no momento.
            </p>
          )}
          {simResult !== null && simResult.length > 0 && (
