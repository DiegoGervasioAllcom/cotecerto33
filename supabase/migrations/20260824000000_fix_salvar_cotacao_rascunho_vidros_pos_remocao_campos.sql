-- ===========================================================================
-- Corrige salvar_cotacao_rascunho após as migrations 20260822000000 e
-- 20260823000000 (remover_campos_redundantes_cotacao /
-- remover_profissao_cotacao_perfil): ambas recriaram a função inteira a
-- partir de uma versão anterior à 20260821010000 (vidros_farois_retrovisores_
-- enum), revertendo `vidros` para `coalesce((...)::boolean,false)`. Como a
-- coluna já é `text` desde 20260821010000, qualquer valor real do enum
-- (ex.: "Não contratada") quebra com "invalid input syntax for type boolean".
--
-- Esta migration reaplica só a correção do `vidros` (via
-- public._normalizar_vidros, criada em 20260821010000) por cima do restante
-- da função já sem os campos removidos por 20260822/20260823 — sem tocar
-- nas migrations antigas.
-- ===========================================================================
...
begin
  ...
  on conflict (cotacao_id) do update set
    ...
    cep_pernoite=excluded.cep_pernoite, tipo_garagem=excluded.tipo_garagem,
