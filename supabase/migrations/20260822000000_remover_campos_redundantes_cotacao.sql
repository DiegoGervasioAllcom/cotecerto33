-- ===========================================================================
-- Remove campos redundantes/resquício do wizard de cotação (22/08/2026)
--
-- Campos identificados como não usados no fluxo atual do wizard e removidos
-- por decisão de produto:
-- - cotacao_veiculo.blindado (boolean) — substituído pelo bloco de blindagem
--   (blindagem/cobertura_blindagem/valor_blindagem/com_franquia_blindagem).
-- - cotacao_veiculo.uso_comercial (text) — substituído por tipo_uso/uso_trabalho/
--   uso_estudo/uso_comercial_dois_dias.
-- - cotacao_veiculo.num_passageiros (text) — resquício sem uso no formulário.
-- - cotacao_coberturas.franquia (text) — substituído por franquia_primeira_opcao/
--   franquia_segunda_opcao.
-- - cotacao_coberturas.dmh (text) — resquício sem uso no formulário.
-- ===========================================================================

-- ---------- drop das constraints de tamanho ----------
alter table public.cotacao_veiculo
  drop constraint if exists cotacao_veiculo_uso_comercial_tamanho,
  drop constraint if exists cotacao_veiculo_num_passageiros_tam;

alter table public.cotacao_coberturas
  drop constraint if exists cotacao_coberturas_franquia_tamanho,
  drop constraint if exists cotacao_coberturas_dmh_tamanho;

-- ---------- drop das colunas ----------
alter table public.cotacao_veiculo
  drop column if exists blindado,
  drop column if exists uso_comercial,
  drop column if exists num_passageiros;

alter table public.cotacao_coberturas
  drop column if exists franquia,
  drop column if exists dmh;

-- ---------- salvar_cotacao_rascunho sem os campos removidos ----------
...
begin
  ...
  insert into public.cotacao_veiculo(
    ...
    alienado, banco, km_mensal, fipe_valor,
    ...
    possui_antifurto_porto, hdi_seguros_basico, antifurto, antifurto_detalhes, cep_circulacao,
    blindagem, cobertura_blindagem, valor_blindagem, com_franquia_blindagem,
  ...
  values (_cot_id,
    ...
    p_payload->'veiculo'->>'cep_circulacao',
  ...
  on conflict (cotacao_id) do update set
    ...
    cep_circulacao=excluded.cep_circulacao,
  ...
  insert into public.cotacao_coberturas(
    cotacao_id, tipo_cobertura, casco, casco_valor, app_morte, app_invalidez, rcf_dm, rcf_dc,
  ...
  values (_cot_id,
    ...
    p_payload->'coberturas'->>'casco_valor',
    p_payload->'coberturas'->>'app_morte', p_payload->'coberturas'->>'app_invalidez',
    p_payload->'coberturas'->>'rcf_dm', p_payload->'coberturas'->>'rcf_dc',
  ...
  on conflict (cotacao_id) do update set
    ...
    app_morte=excluded.app_morte, app_invalidez=excluded.app_invalidez,
    rcf_dm=excluded.rcf_dm, rcf_dc=excluded.rcf_dc,
