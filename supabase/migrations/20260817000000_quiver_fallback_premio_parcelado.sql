-- 20260817000000 — registrar_premios_quiver: aceitar prêmio via "parcelas"
-- quando "avista" não existir.
--
-- Bug real (produção, 17/08/2026): a função só lia opcoes[0]->>'avista' pra
-- extrair o valor do prêmio. Produtos que só oferecem parcelado, sem preço
-- à vista (ex.: Suhai "Roubo e Furto c/ Assistência", parcelas: "em 12x de
-- R$ 463,20"), ficavam com premio_txt null — o card era descartado como
-- inválido, premios_validos zerava mesmo com temPremios:true vindo do robô,
-- e o webhook gravava status='erro_quiver' com a mensagem "A seguradora não
-- retornou prêmios válidos para esta cotação.", descartando um resultado
-- real e correto (5 cards da Suhai, cotação 155032).
--
-- Fallback: se avista ausente/vazio, extrai o valor de "parcelas" (formato
-- "em Nx de R$ X.XXX,XX" ou "de R$ X.XXX,XX"), mesma normalização pt-BR já
-- usada para avista (remove tudo exceto dígito e vírgula antes de validar).
-- Resto da função inalterado (trava de linha, idempotência, preservação de
-- sucesso anterior).
...
begin
  ...
  if _tem_premios then
    ...
    loop
      ...
      if _premio_txt is null then
        -- Sem preço à vista (produto só parcelado): tenta extrair o valor da
        -- parcela em "parcelas" (ex.: "em 12x de R$ 463,20"). Usa só o
        -- primeiro número com vírgula decimal da string, pra não pegar o
        -- "12" da quantidade de parcelas.
        _premio_txt := (regexp_match(coalesce(_opcao->>'parcelas', ''), '([0-9\.]*[0-9],[0-9]{1,2})'))[1];
        if _premio_txt is not null then
          _premio_txt := regexp_replace(_premio_txt, '[^0-9,]', '', 'g');
        end if;
      end if;
