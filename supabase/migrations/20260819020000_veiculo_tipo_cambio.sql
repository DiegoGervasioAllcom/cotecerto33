-- ===========================================================================
-- Campo "Tipo de Câmbio" do veículo — robô Playwright (19/08/2026)
--
-- O robô já sabe preencher o campo "Tipo de Câmbio" no portal quando o
-- portal não consegue inferi-lo sozinho pela placa (ex.: placa FRI0481) —
-- mas só usa o valor se ele vier em `veiculo.tipoCambio` no payload de
-- /cotacao ("Manual", "Automático" ou "Semiautomático"). Sem isso, a
-- cotação falha com erro genérico quando o portal exige o campo. Este
-- fix adiciona a coluna em cotacao_veiculo, o campo no formulário
-- (StepVeiculo) e o mapeamento no payload de /cotacao (quiver.functions.ts).
-- ===========================================================================

alter table public.cotacao_veiculo
  add column if not exists tipo_cambio text
    check (tipo_cambio is null or tipo_cambio in ('Manual', 'Automático', 'Semiautomático'));
