-- ============================================================
-- Zero km completo: campos condicionais mapeados no robô em 17/08/2026
-- (#Padrao2000_Cobertura1405 / #Padrao2000_Cobertura7003), revelados só
-- quando zeroKm=Sim e obrigatórios nesse caso. `zeroKm` em si já existia
-- (cotacao_veiculo.zero_km); faltavam os dois subcampos.
-- ============================================================
...
comment on column public.cotacao_veiculo.data_saida_concessionaria is
  'Obrigatório no robô quando zero_km=true (Padrao2000_Cobertura1405, formato DD/MM/AAAA no payload).';
comment on column public.cotacao_veiculo.odometro is
  'Obrigatório no robô quando zero_km=true (Padrao2000_Cobertura7003, somente dígitos no payload) — decisão fail-closed do robô, revisar se o portal confirmar que não é sempre obrigatório.';

-- ---------- salvar_cotacao_rascunho: persistir os 2 campos novos ----------
...
begin
  select id, empresa_id, responsavel_id into _cot_id, _empresa_id, _responsavel_id
  from public.cotacoes where id = p_cotacao_id;

  if _cot_id is null then
    insert into public.cotacoes (id, empresa_id, responsavel_id, lead_id, status)
    values (
      p_cotacao_id,
      (p_payload->>'empresa_id')::uuid,
    ...
    update public.cotacoes set atualizado_em = now() where id = _cot_id;
  end if;

  insert into public.cotacao_segurado(
    cotacao_id, cpf_cnpj, nome, nome_social, nasc, sexo, estado_civil,
    celular, tel_res, email, cep, logradouro, bairro, cidade, uf, sms_optin
  )
  values (_cot_id,
    p_payload->'segurado'->>'cpf_cnpj', p_payload->'segurado'->>'nome',
    p_payload->'segurado'->>'nome_social',
    nullif(p_payload->'segurado'->>'nasc','')::date,
    p_payload->'segurado'->>'sexo', p_payload->'segurado'->>'estado_civil',
    p_payload->'segurado'->>'celular', p_payload->'segurado'->>'tel_res',
    p_payload->'segurado'->>'email', p_payload->'segurado'->>'cep',
    p_payload->'segurado'->>'logradouro', p_payload->'segurado'->>'bairro',
  ...
  on conflict (cotacao_id) do update set
    cpf_cnpj=excluded.cpf_cnpj, nome=excluded.nome, nome_social=excluded.nome_social,
    nasc=excluded.nasc, sexo=excluded.sexo, estado_civil=excluded.estado_civil,
  ...
  insert into public.cotacao_seguro(
    cotacao_id, tipo_seguro, ramo, categoria, vig_ini, vig_fim,
    cia_atual, apolice_atual, ci_atual, classe_bonus,
    sucursal_anterior, cobertura_anterior, status_apolice_anterior,
    inicio_vigencia_anterior, fim_vigencia_anterior,
    seguradoras_sel, tipo_calculo, tipo_cobertura, grupo_producao, campanha, observacoes
  )
  values (_cot_id,
    p_payload->'seguro'->>'tipo_seguro', p_payload->'seguro'->>'ramo',
    p_payload->'seguro'->>'categoria',
    nullif(p_payload->'seguro'->>'vig_ini','')::date,
    nullif(p_payload->'seguro'->>'vig_fim','')::date,
    ...
    (select array_agg(x) from jsonb_array_elements_text(coalesce(p_payload->'seguro'->'seguradoras_sel','[]'::jsonb)) x),
  ...
  values (_cot_id,
    ...
    p_payload->'perfil'->>'seguro_corretor_proximo',
    (p_payload->'perfil'->>'jovens_18_25')::boolean,
  ...
  insert into public.cotacao_coberturas(
    cotacao_id, tipo_cobertura, casco, casco_valor, franquia,
    app_morte, app_invalidez, dmh, rcf_dm, rcf_dc, vidros, carro_reserva, assist_24,
    modalidade, percentual_ajuste, franquia_primeira_opcao, franquia_segunda_opcao,
    danos_morais, despesas_extras, pequenos_reparos, mais_assistencias, mais_assistencias_seguradora,
  ...
  values (_cot_id,
    ...
    (p_payload->'coberturas'->>'vidros')::boolean, p_payload->'coberturas'->>'carro_reserva',
    p_payload->'coberturas'->>'assist_24',
  ...
  on conflict (cotacao_id) do update set
    ...
    dmh=excluded.dmh, rcf_dm=excluded.rcf_dm, rcf_dc=excluded.rcf_dc, vidros=excluded.vidros,
    carro_reserva=excluded.carro_reserva, assist_24=excluded.assist_24,
    ...
    pequenos_reparos=excluded.pequenos_reparos, mais_assistencias=excluded.mais_assistencias,
    mais_assistencias_seguradora=excluded.mais_assistencias_seguradora,
