-- V11.2.2 — snapshot tipado das sete variantes de boas-vindas.
-- Link/token de recovery continuam fora do banco e são adicionados em memória.
...
begin
  ...
  select p.id, p.nome, p.email, p.cargo_id, cg.nome, p.superior_id,
         e.nome, e.cidade, e.uf, e.parent_id, e.convite_id,
         mf.nome, mf.modalidade
    into _profile_id, _nome, _email, _cargo_id, _cargo, _superior_id,
         _empresa_nome, _cidade, _uf, _parent_id, _convite_id,
         _modelo, _modalidade
  ...
  select coalesce(
    ...
    (select nullif(trim(p.nome), '') from public.profiles p
      where p.empresa_id = coalesce(_vinc_empresa_id, _parent_id)
...
comment on function public.enfileirar_boas_vindas(uuid) is
  'V11.2.2: cria snapshot tipado de uma das sete boas-vindas, sem persistir link ou token.';
