-- ===========================================================================
-- Lead Manual — origem: o gate "Lead Manual — origem" (front, novo-lead.tsx)
-- captura o canal real (Movida/Google/Facebook/Indicação/Manual/Outro) antes
-- do wizard, espelhando o protótipo. Sem isto, todo lead criado por aqui
-- gravava `origem = 'cotacao'` fixo — o gatilho `trg_leads_resolver_canal`
-- (V11.0.4) então resolvia sempre para o canal "Cotação direta", mesmo
-- quando o vendedor sabia que o lead veio de indicação/Google/etc.
--
-- `p_origem` é opcional (default 'cotacao') pra não quebrar quem já chama a
-- RPC sem essa origem (ex.: fluxo de "Atender agora"); só é usado na criação
-- do lead (ramo `if _cot_id is null`) — updates de um rascunho já existente
-- não tocam o canal do lead. Corpo idêntico ao da G4.1
-- (20260723180000_g4_1_coberturas_plano_assistencias_descontos.sql), só com
-- o parâmetro novo e a linha marcada abaixo.
-- ===========================================================================

-- Assinatura muda (ganha p_origem): drop explícito evita ambiguidade de
-- overload com a versão de 2 parâmetros nas chamadas via RPC.
...
begin
  ...
  if _cot_id is null then
    ...
    -- Única mudança de negócio desta migration: `coalesce(p_origem,'cotacao')`
    -- em vez do literal 'cotacao' fixo.
