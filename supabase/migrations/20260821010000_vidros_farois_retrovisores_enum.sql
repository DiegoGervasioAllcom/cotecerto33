-- ===========================================================================
-- "Vidros, faróis e retrovisores" — vira enum de 4 níveis (21/08/2026)
--
-- O front trocou esse campo de switch boolean pra select de 4 níveis
-- ("Não contratada"/"Básico"/"Intermediário"/"Superior") pra bater com o
-- enum que o robô Playwright da Quiver espera (vidrosFarosRetrovisores).
-- A coluna `cotacao_coberturas.vidros` continuava `boolean`, então o nível
-- exato escolhido não sobrevivia a um reload do rascunho (ver
-- doc/PLANO-INTEGRACAO-7-CAMPOS-ROBO-2026-08.md, Onda 2, item I.3).
--
-- Esta migration:
-- 1. Converte a coluna de boolean pra text, mapeando os dados existentes
--    (true -> 'Superior', false/null -> 'Não contratada' — mesmo mapeamento
--    que o front já fazia defensivamente).
-- 2. Adiciona CHECK restringindo aos 4 valores.
-- 3. Atualiza salvar_cotacao_rascunho pra gravar a string direto (sem
--    ::boolean), normalizando variações de case/acento e tolerando o
--    formato antigo ('true'/'false') vindo de rascunhos ainda em sessão.
-- ===========================================================================
...
begin
  if exists (
    ...
    where table_schema = 'public' and table_name = 'cotacao_coberturas'
      and column_name = 'vidros' and data_type = 'boolean'
  ) then
    alter table public.cotacao_coberturas
      alter column vidros type text
      using (case when vidros then 'Superior' else 'Não contratada' end);
  end if;
end $$;

alter table public.cotacao_coberturas
  alter column vidros set default 'Não contratada';

update public.cotacao_coberturas
   set vidros = 'Não contratada'
 where vidros is null;

do $$
begin
  alter table public.cotacao_coberturas
    add constraint cotacao_coberturas_vidros_check
    check (vidros in ('Não contratada', 'Básico', 'Intermediário', 'Superior'));
exception when duplicate_object then null;
end $$;

-- Função auxiliar de normalização (tolera case/acento e o formato legado
-- boolean 'true'/'false' vindo de rascunhos ainda em memória no front).
create or replace function public._normalizar_vidros(p_valor text)
returns text
language plpgsql
immutable
as $$
declare
  _v text := lower(trim(coalesce(p_valor, '')));
begin
  if _v = '' or _v in ('nao contratada', 'não contratada', 'false') then
    return 'Não contratada';
  elsif _v in ('basico', 'básico') then
    return 'Básico';
  elsif _v in ('intermediario', 'intermediário') then
    return 'Intermediário';
  elsif _v in ('superior', 'true') then
    return 'Superior';
  else
    return 'Não contratada';
  end if;
...
begin
  ...
  insert into public.cotacao_veiculo(
    ...
    acessorios, kit_acessorios, opcionais, equipamentos, acessorios_detalhes, tipo_cambio
  )
  values (_cot_id,
    ...
    nullif(p_payload->'veiculo'->>'tipo_cambio', '')
  )
  on conflict (cotacao_id) do update set
  ...
  values (_cot_id,
    ...
    public._normalizar_vidros(p_payload->'coberturas'->>'vidros'),
