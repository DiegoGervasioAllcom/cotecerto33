-- ===========================================================================
-- Corrige aprovar_acesso: vendedor de Franquia Full nunca herdava empresa_id
--
-- QA manual (10/08/2026) fazendo o ciclo completo "Analisar → Liberar acesso"
-- na tela da Full e depois testando "Excluir": a exclusão sempre falhava com
-- "Vendedor não pertence à sua Franquia Full", porque `fn_full_dona_vendedor`
-- exige `v.empresa_id = f.empresa_id` — e `aprovar_acesso` só atualizava
-- `superior_id`, nunca `empresa_id`. O vendedor ficava para sempre com a
-- empresa "pendente" criada no próprio cadastro (signup), nunca com a empresa
-- compartilhada da Franquia Full.
--
-- O "Cadastro direto" (fn_cadastrar_vendedor_full, migration
-- 20260809212122) já fazia isso certo (`empresa_id=_empresa` no insert) — só
-- o caminho por Convite Supper (aprovar_acesso) estava faltando. Esta
-- migration replica a mesma regra: quando o pedido aprovado é um vendedor com
-- superior apontando para uma Franquia Full, empresa_id passa a acompanhar a
-- empresa do superior.
-- ===========================================================================
...
declare
  ...
  _modalidade text; _empresa_full uuid;
begin
