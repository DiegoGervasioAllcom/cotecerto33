-- ===========================================================================
-- Corrige overload duplicado de salvar_cotacao_rascunho
--
-- QA manual (18/08/2026): salvar rascunho de uma cotação de lead NÃO manual
-- (assumido via pipeline/distribuição, sem LeadManualGate) falhava com
-- "Erro ao salvar — tente novamente". Causa: 20260817050000_veiculo_
-- zerokm_completo.sql criou `salvar_cotacao_rascunho(uuid, jsonb)` — 2
-- parâmetros — em cima de uma cópia ANTIGA da função (sem p_origem, sem
-- criação de lead/cotação nova, com nomes de coluna desatualizados como
-- `nasc` em vez de `nascimento`), sem dropar a versão de 3 parâmetros já
-- existente (20260817020000, com p_origem default 'cotacao'). As duas
-- passaram a coexistir como overloads. Front sempre chama com os 3
-- argumentos nomeados (p_cotacao_id, p_payload, p_origem), mas quando
-- p_origem vem undefined (lead não manual pula o LeadManualGate, então
-- f.canalOrigem nunca é preenchido — ver useCotacaoRascunho.ts),
-- JSON.stringify remove a chave e a chamada chega ao PostgREST com só 2
-- argumentos nomeados — ambíguo entre as duas funções, PostgREST devolve
-- "function ... is not unique" e o front mostra o erro genérico.
--
-- Correção: remove a versão de 2 parâmetros (errada e desatualizada) e
-- reincorpora os 2 campos de Zero Km (data_saida_concessionaria, odometro)
-- na versão de 3 parâmetros, que é a única que deve existir daqui pra
-- frente — corpo idêntico ao de 20260817020000_salvar_cotacao_rascunho_
-- apolice_anterior_full.sql, só com esses 2 campos adicionados no insert/
-- update de cotacao_veiculo.
-- ===========================================================================
