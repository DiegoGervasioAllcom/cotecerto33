...
/**
 * Teste de caracterização da query de `/venda/em-cotacao` (T4, Pipeline V12).
 *
 * Captura a query ATUAL de `fetchEmCotacaoRows` (extraída do `useEffect` de
 * `em-cotacao.tsx` só para viabilizar este teste — o projeto não tem
 ...
 * hardcoded (`"rascunho"`) por `EM_COTACAO_STATUSES` (de `@/lib/lead-etapa`)
 * e confirmar que o filtro efetivo não mudou.
 */
...
import { fetchEmCotacaoRows } from "@/routes/_authenticated/venda/em-cotacao";
import { EM_COTACAO_STATUSES } from "@/lib/lead-etapa";

describe("query de cotações em preenchimento (/venda/em-cotacao)", () => {
  beforeEach(() => {
  ...
  it("filtra cotacoes pelos status de EM_COTACAO_STATUSES, ordenadas por atualizado_em desc, limitadas a 200", async () => {
    ...
    // Filtro efetivo de status: hoje é `.eq("status", "rascunho")` — mesmo
    // valor que `EM_COTACAO_STATUSES` (array de 1 item) carrega hoje.
    expect(EM_COTACAO_STATUSES).toEqual(["rascunho"]);
    ...
    if (Array.isArray(statusFiltrado)) {
      expect(statusFiltrado).toEqual([...EM_COTACAO_STATUSES]);
    } else {
      expect(statusFiltrado).toBe(EM_COTACAO_STATUSES[0]);
    }
  });
