...
describe("distribuição automática de leads", () => {
  ...
  beforeAll(async () => {
    ...
    const { data: cfg } = await admin.from("distribuicao_config").select("*").eq("id", "default").maybeSingle();
    cfgOriginal = cfg;
    await admin.from("distribuicao_config").upsert({
      id: "default", automatico_on: true, modo: "regiao", criterios: { regiao: true },
    });

    // franquia aprovada na cidade única + vendedor aprovado nela
    const { data: emp, error: eEmp } = await admin.from("empresas").insert({
      nome: uniq("Franquia Dist"), tipo: "pj", documento: uniqDoc(),
      status: "aprovada", uf: UF, cidade: cidadeUnica,
    }).select("id").single();
    if (eEmp) throw eEmp;
    empresaId = emp.id;
    ...
    vendedorId = userId;
    await admin.from("profiles").update({ empresa_id: empresaId, status: "aprovada" }).eq("id", vendedorId);
  ...
  it("lead com cidade casada é distribuído: empresa, responsável e timestamp", async () => {
    const { data: lead, error } = await matriz.from("leads").insert({
      nome: uniq("Lead SP"), origem: "teste", dados: { cidade: cidadeUnica },
    }).select("empresa_id,responsavel_id,distribuido_em").single();
  ...
  it("NEGATIVO: automático desligado → lead fica sem destino", async () => {
    ...
    const { data: lead } = await matriz.from("leads").insert({
      nome: uniq("Lead sem dist"), origem: "teste", dados: { cidade: cidadeUnica },
    }).select("empresa_id,responsavel_id").single();
  ...
  it("NEGATIVO: em avaliação da matriz → trigger não age", async () => {
    const { data: lead } = await matriz.from("leads").insert({
      nome: uniq("Lead avaliacao"), origem: "teste", dados: { cidade: cidadeUnica }, em_avaliacao_matriz: true,
    }).select("empresa_id,responsavel_id").single();
    expect(lead?.empresa_id).toBeNull();
    expect(lead?.responsavel_id).toBeNull();
  });
